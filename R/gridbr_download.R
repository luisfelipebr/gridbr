...
gridbr_download <- function(input, cellsize, census_data = TRUE, equal_area = FALSE){
  ...
  if (suppressWarnings(tryCatch({readLines("http://example.com/",n=1)}, error = function(e) FALSE)) == FALSE & "gridbr.data" %in% utils::installed.packages() == FALSE) {
  ...
  if (sf::st_bbox(input)[[1]] < 2800000 |
      ...
      sf::st_bbox(input)[[4]] > 7350000+(10*500000)) stop('the input is outside IBGE Statistical Grid coverage.')
  ...
  if (cellsize == "200M") {
    ...
    # make the grid
    grid1 <- sf::st_make_grid(x = input1,
                              cellsize = 200,
                              crs = gridbr_crs,
                              what = "polygons",
                              square = TRUE,
    ...
    # make the grid
    grid2 <- sf::st_make_grid(x = input2,
                              cellsize = 1000,
                              crs = gridbr_crs,
                              what = "polygons",
                              square = TRUE,
    ...
    cellsize2 <- as.numeric(gsub(pattern = "KM",
                                 replacement = "000",
                                 x = cellsize))
